...
from src.core.data_processor import FieldMergeMode, ProcessingStats
...
class ProductImporter(BaseImporter):
    ...
    def validate_input_data(self, df: pd.DataFrame) -> bool:
        ...
        if null_item_names > 0:
            self.logger.warning(f"⚠️  Found {null_item_names} records with null item names")
        
        deleted_items = df[df['Item Name'] == 'Deleted Item'].shape[0]
        if deleted_items > 0:
            self.logger.info(f"🗑️  Found {deleted_items} deleted items (will be skipped)")
    ...
    def preprocess_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Net Qty' in df.columns:
            ...
            df['current_stock'] = 0
            self.logger.warning("⚠️  No quantity fields found, setting current_stock to 0")
    ...
    def get_or_create_brand(self, brand_name: str) -> Optional[str]:
        ...
        try:
            ...
            if not brand_obj:
                ...
                self.logger.debug(f"🏷️  Created new brand: {brand_name}")
    ...
    def parse_gst_and_cess(self, gst_raw: Any) -> Tuple[int, Optional[int]]:
        ...
        try:
            ...
            self.logger.warning(f"⚠️  Cannot parse GST '{gst_str}': {e}")
            return 0, None
    ...
    def process_row(self, row: pd.Series) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.warning(f"⚠️  Error processing product row: {e}")
            return None
